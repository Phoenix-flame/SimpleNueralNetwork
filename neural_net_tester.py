...
def main(neural_net_func, data_sets, rate=1.0, max_iterations=500):
    verbose = True
    for name, training_data, test_data in data_sets:
        ...
        result = test(nn, test_data, verbose=True)
        print("Accuracy: %f"%(result))

            
        result = test(nn, training_data, verbose=True)
        print("Accuracy: %f"%(result))
...
if __name__=="__main__":
    ...
    for test_name in test_names:
        if test_name == "simple":
            ...
            main(make_neural_net_large, two_moons_data_set, max_iterations=100)
